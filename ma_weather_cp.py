# -*- coding: utf-8 -*-
"""
Created on Sun Oct 27 18:08:05 2024
...
loc_name = pd.read_csv('C:/Users/scott/Desktop/anl-outage-data/data/location_with_totalcustomers/ma_loc_correction.csv').iloc[:,[0]].values
obs_outage, obs_weather, locs, _ = dataloader(config["MA Mar 2018"], outageN=1, weatherN=1, isproj=False)
...
startt     = arrow.get(config["MA Mar 2018"]["_startt"], "YYYY-MM-DD HH:mm:ss")
endt       = arrow.get(config["MA Mar 2018"]["_endt"], "YYYY-MM-DD HH:mm:ss")
train_start = arrow.get('2018-03-01 00:00:00', "YYYY-MM-DD HH:mm:ss")
train_end = arrow.get('2018-03-07 00:00:00', "YYYY-MM-DD HH:mm:ss")
...
if os.path.exists("hawkes_ma_20180301_0307.pt"):
    model = TorchHawkesNNCovariates(d=24, obs=obs_outage, covariates=obs_weather)
    model.load_state_dict(torch.load("hawkes_ma_20180301_0307.pt"))
else:
    train(model, locs=locs[:,:2], k=50, niter=300, lr=1., log_interval=10)
    torch.save(model.state_dict(),"hawkes_ma_20180301_0307.pt")
    model = TorchHawkesNNCovariates(d=24, obs=obs_outage, covariates=obs_weather)
    model.load_state_dict(torch.load("hawkes_ma_20180301_0307.pt"))
...
plt.title('Prediction for MA')
plt.legend()
plt.tight_layout()
plt.show()


test_start = arrow.get('2018-03-12 00:00:00', "YYYY-MM-DD HH:mm:ss")
test_end = arrow.get('2018-03-20 00:00:00', "YYYY-MM-DD HH:mm:ss")
...
# calibration set
window_size = 50
cal_start1 = arrow.get('2018-03-07 00:00:00', "YYYY-MM-DD HH:mm:ss")
cal_end1 = arrow.get('2018-03-10 00:00:00', "YYYY-MM-DD HH:mm:ss")
cal_start2 = arrow.get('2018-03-20 00:00:00', "YYYY-MM-DD HH:mm:ss")
cal_end2 = arrow.get('2018-03-31 00:00:00', "YYYY-MM-DD HH:mm:ss")
startind_cal1   = int((cal_start1.timestamp() - startt.timestamp()) / (endt.timestamp()-startt.timestamp())*(obs_outage.shape[1]-1))
endind_cal1    = int((cal_end1.timestamp() - startt.timestamp()) / (endt.timestamp()-startt.timestamp())*(obs_outage.shape[1]-1))
startind_cal2   = int((cal_start2.timestamp() - startt.timestamp()) / (endt.timestamp()-startt.timestamp())*(obs_outage.shape[1]-1))
endind_cal2    = int((cal_end2.timestamp() - startt.timestamp()) / (endt.timestamp()-startt.timestamp())*(obs_outage.shape[1]-1))

cal_data1 = obs_outage[:,startind_cal1:endind_cal1+1] 
cal_data2 = obs_outage[:,startind_cal2:endind_cal2+1] 
cal_pred1 = lams[:,startind_cal1:endind_cal1+1] 
cal_pred2 = lams[:,startind_cal2:endind_cal2+1] 
cal_data = np.concatenate([cal_data1,cal_data2],axis=1)
cal_pred = np.concatenate([cal_pred1,cal_pred2],axis=1)
...
# visualization
# ma  j=[10,11,70,80]
plt_start = arrow.get('2018-03-12 00:00:00', "YYYY-MM-DD HH:mm:ss")
plt_end = arrow.get('2018-03-20 00:00:00', "YYYY-MM-DD HH:mm:ss")
